    ...
    {!enrolledCourses ? (
      ...
      <div className="space-y-4">
        ...
        {enrolledCourses.map((course, index) => (
          <div
            key={index}
            className="group cursor-pointer flex flex-col md:flex-row md:items-center gap-4 p-4 bg-[#1e2536] rounded-xl border border-[#2a3245] hover:border-indigo-500/50 transition-all duration-300 hover:shadow-lg hover:shadow-indigo-500/10"
          >
            {/* Course Info */}
            <div className="flex items-start gap-4 w-full md:w-1/3">
              <div className="relative">
                <img
                  ...
                  className="w-20 h-20 object-cover rounded-lg border border-[#2a3245] group-hover:border-indigo-500/30 transition-colors"
                />
